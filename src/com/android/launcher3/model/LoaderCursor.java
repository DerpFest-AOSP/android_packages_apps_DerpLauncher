...
public class LoaderCursor extends CursorWrapper {
    ...
    protected boolean checkItemPlacement(ItemInfo item) {
        ...
        if (!mOccupied.containsKey(item.screenId)) {
            ...
            if (Utilities.showQuickspace(mContext) && item.screenId == Workspace.FIRST_SCREEN_ID) {
                // Mark the first X columns (X is width of the quickspace container) in the first row as
                // occupied (if the feature is enabled) in order to account for the quickspace
                // container.
